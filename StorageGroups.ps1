function Create-StorageGroupRoot {	
	param( 	$AccessControlMaps	
		 )
	$Members=@()
	foreach ($Map in $AccessControlMaps)
		{	$LocalMembers = @( @{	'@odata,id'		=	'/redfish/v1/StorageServices/'+$NimbleSerial+'/StorageGroups/'+$Map.id
                                }
                             )
	...
	$SGRoot = @{	'@Redfish.Copyright'	= 	$RedfishCopyright;
					...
					'@odata.id'				=	'/redfish/v1/StorageServices/'+$NimbleSerial+'/StorageGroups';
    ...
    FolderAndFile $SGRoot ("StorageServices\"+$NimbleSerial+"\StorageGroups")
}
		
function Create-StorageGroupsIndex {
    param( 	$AccessControlMaps
		 )
	foreach ($Map in $AccessControlMaps)
		{	$ServerEPG=@()
            ...
            $SG = @{	'@Redfish.Copyright'    = 	$RedfishCopyright;
						...
						'@odata.id'             =	'/redfish/v1/StorageServices/'+$NimbleSerial+'/StorageGroups/'+$Map.id;
                ...
                FolderAndFile $SG $("StorageServices\"+$NimbleSerial+"\StorageGroups\"+$Map.id)
		}
}
