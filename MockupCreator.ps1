...
{   param(  [Parameter(Mandatory = $True)]
    ...
    # write-host $FileData
    New-Item -path $Folder -name 'index.json' -value $FileData     
...
# StorageSystem ####
    ...
    {   $PathRaw=$MyArray.'@odata.id'
        ...
        {   $PathRaw=$MyVol.'@odata.id'
            ...
            write-host "The path is $MyVolPath and the Volume is $MyVolSplit"
            ...
            {   $Data= ( Get-SFSnapshotIndex $MyVolSplit | ConvertTo-JSON -depth 10 )
                ...
                {   $PathRaw=$MySnap.'@odata.id'
                    ...
                    write-host "The path is $MySnapPath and the Snapshot is $MySnapSplit"
